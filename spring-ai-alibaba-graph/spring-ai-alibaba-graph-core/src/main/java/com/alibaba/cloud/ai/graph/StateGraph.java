...
public class StateGraph {
    ...
    public StateGraph addSubgraph(String id, CompiledGraph subGraph) throws GraphStateException {
        return addNode(id, (AsyncNodeActionWithConfig<Object, Map>) (state, config) -> {
            ObjectMapper mapper = new ObjectMapper();
            AsyncGenerator<NodeOutput> generator = subGraph.stream(mapper.convertValue(state, Map.class), config);
            return CompletableFuture.supplyAsync((Supplier<Map>) () -> Map.of(NodeState.SUB_GRAPH, generator));
        });
    }
    ...
    public CompiledGraph compile() throws GraphStateException {
        return compile(CompileConfig.builder().saverConfig(new SaverConfig().register(SaverConstant.MEMORY, new MemorySaver())).build());
    }
